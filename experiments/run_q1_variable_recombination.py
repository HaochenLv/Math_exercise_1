...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/q1/variable_recombination_v8"),
    )
    ...
    result = Q1VariableFlightCountRecombinationSolver().solve_with_diagnostics(problem)
    if not result.converged:
        raise RuntimeError("V8 hit max_iterations before convergence")
    ...
    comparison = {
        "three_route_recombination_v7_start": starting_dict,
        "variable_flight_count_recombination_v8": metric_dict,
        ...
        "flight_count_change": (
            metrics.number_of_flights - result.starting_metrics.number_of_flights
        ),
        "number_of_variable_recombination_moves": len(result.decisions),
        ...
        "number_of_cross_airport_polish_moves": len(
            result.cross_airport_polish_decisions
        ),
        "number_of_three_route_polish_moves": len(
            result.three_route_polish_decisions
        ),
    ...
    with (output_dir / "variable_recombination_moves.csv").open(
        ...
        writer.writerow(
            [
                ...
                "cross_airport_polish_count",
                "three_route_polish_count",
            ]
        )
        for decision in result.decisions:
            writer.writerow(
                [
                    ...
                    " | ".join(
                        ";".join(order) for order in decision.new_service_orders
                    ),
                    decision.block_polish_move_count,
                    decision.partial_polish_move_count,
                    decision.generalized_tail_polish_count,
                    decision.cross_airport_polish_count,
                    decision.three_route_polish_count,
                ]
            )

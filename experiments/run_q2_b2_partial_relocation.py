...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/q2/b2_partial_relocation"))
    parser.add_argument("--packing-time-limit", type=float, default=30.0)
    parser.add_argument("--max-hosts-per-donor", type=int, default=32)
    ...
    b1 = Q2ResidualTailEliminationSolver(
        max_hosts_per_donor=24
    ...
    result = Q2PartialPassengerRelocationSolver(
        ...
        tail_max_hosts_per_donor=args.max_hosts_per_donor,
    ...
    partial_direct_savings = sum(
        decision.aircraft_savings_minutes for decision in result.partial_relocations
    )
    comparison = {
        "stage": "Q2-B2 partial passenger relocation + B1 polish",
        "q1_anchor": _metrics_dict(q1_metrics),
        "b0b_complete": _metrics_dict(b0b.metrics),
        "b1_complete": _metrics_dict(b1.metrics),
        "b2_complete": metrics,
        "partial_relocation_count": len(result.partial_relocations),
        "partial_relocation_direct_aircraft_savings_minutes": partial_direct_savings,
        ...
        "tail_polish_total_aircraft_savings_minutes": result.tail_polish_aircraft_savings_minutes,
    ...
    with (output_dir / "partial_relocations.csv").open("w", newline="", encoding="utf-8") as handle:
        rows = [asdict(decision) for decision in result.partial_relocations]
        fieldnames = list(rows[0]) if rows else [
            "donor_flight_uid", "host_flight_uid", "donor_kind", "moved_passenger_count",
            "donor_passenger_count_before", "donor_minutes_before", "host_minutes_before",
            "donor_minutes_after", "host_minutes_after", "aircraft_savings_minutes",
            "passenger_minutes_delta", "fuel_kg_delta", "donor_aircraft_type_after",
            "host_aircraft_type_after", "donor_base_airport_after", "host_base_airport_after",
            "donor_service_order_after", "host_service_order_after"
        ]
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for row in rows:
            row["donor_service_order_after"] = "->".join(row["donor_service_order_after"])
            row["host_service_order_after"] = "->".join(row["host_service_order_after"])
            writer.writerow(row)
        ...
        elimination_rows.append(
            {
                ...
                "recipients": ";".join(
                    f"{r.flight_uid}:{r.passenger_count}:{r.added_aircraft_minutes}:"
                    f"{r.after_aircraft_type}:{r.after_base_airport}:{'->'.join(r.after_service_order)}"

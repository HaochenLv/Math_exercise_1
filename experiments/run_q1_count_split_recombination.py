...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/q1/count_split_recombination_v10"),
    )
    ...
    result = Q1ExactCountSplitPairSolver().solve_with_diagnostics(problem)
    if not result.converged:
        raise RuntimeError("V10 hit max_iterations before convergence")
    ...
    comparison = {
        "four_route_recombination_v9_start": starting_dict,
        "exact_count_split_pair_v10": metric_dict,
        ...
        "number_of_count_split_moves": len(result.decisions),
    ...
    with (output_dir / "count_split_moves.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(
            [
        ...
        for decision in result.decisions:
            writer.writerow(
                [
                    ...
                    decision.generalized_tail_polish_moves,
                ]
            )

...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/q1/recombination_v7"),
    )
    ...
    result = Q1ThreeRouteRecombinationSolver().solve_with_diagnostics(problem)
    if not result.converged:
        raise RuntimeError("V7 hit max_iterations before convergence")
    ...
    comparison = {
        "land_cross_airport_local_search_v6_start": starting_dict,
        "three_route_recombination_v7": metric_dict,
        ...
        "number_of_recombination_moves": len(result.decisions),
    ...
    with (output_dir / "recombination_moves.csv").open(

...
def _toy_problem_and_solution() -> tuple[ProblemData, Solution]:
    facilities = tuple(f"F{i:03d}" for i in range(1, 9))
...
def test_four_route_recombination_resolves_four_way_cycle():
    problem, starting = _toy_problem_and_solution()
    start_metrics = evaluate_solution(problem, starting)

    result = Q1FourRouteRecombinationSolver(
    ...
    assert result.metrics.number_of_flights == 4
    ...
    for pair in (
        ...
        assert frozenset(pair) in destination_sets
